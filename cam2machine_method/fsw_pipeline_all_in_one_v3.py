...
方法更新（最近表面 + 中轴线）
----------------------------
- 最近表面：从高度图中按分位参考z与厚度margin提取薄层，并做形态学清理与最大连通域筛选，避免把远处背景/孔洞误判为目标；
- 中轴线：沿用增强版 `fit_centerline_plus`（桥接→骨架→最长路→参考路径评分→RDP/SG/等弧长），只更换输入掩码为“最近表面薄层”。
...
CONFIG: Dict = dict(
    ...
    # 机床系 ROI（后裁剪用）
    roi_center_xy=[45.0, 45.0],  # 机床 ROI 中心（mm）
    roi_size_mm=150.0,  # 机床 ROI 边长（mm）

    # 相机像素 ROI（后裁剪用）
    cam_roi_xywh=[714,839,228,191],  # (x,y,w,h)，单位：像素
    ...
    morph_open=3,
    morph_close=5,
    depth_margin_mm=30.0,  # 最近表面厚度（mm），围绕参考z的上/下带
    nearest_use_percentile=True,  # 使用分位数估计参考z（更稳健）
    nearest_qlo=1.0,  # 当 z_select='min' 时用的低分位（%）
    nearest_qhi=99.0,  # 当 z_select='max' 时用的高分位（%）
    min_component_area_px=600,  # 最近表面连通域最小面积（px）
...
def extract_nearest_surface_mask_v3(
    height: np.ndarray,
    valid_mask: np.ndarray,
    *,
    z_select: str = 'min',              # 'min' 表示“离相机最近”为较小z；若你的高度图越大越近，请改为 'max'
    depth_margin_mm: float = 3.0,       # 最近表面薄层厚度（mm）
    roi_rect_px: tuple | None = None,   # (x, y, w, h)，仅在 ROI 内估计与提取
    open_k: int | None = 3,             # 形态学开运算核尺寸（像素）
    close_k: int | None = 5,            # 形态学闭运算核尺寸（像素）
    use_percentile: bool = True,        # True: 用分位数估计参考z，更抗噪
    qlo: float = 1.0,                   # 低分位（%），z_select='min' 时使用
    qhi: float = 99.0,                  # 高分位（%），z_select='max' 时使用
    min_component_area_px: int = 600    # 最小连通域面积（像素）
) -> np.ndarray:
    """
    从高度图中提取“最近表面”的薄层掩码（0/255）。

    参数
    ----
    height : (H,W) 浮点高度图，单位 mm，可含 NaN。
    valid_mask : (H,W) 二值或灰度，有效像素掩码（>0 视为有效）。
    z_select : 'min' 或 'max'，决定“最近”的方向：
        - 'min'  : 参考 z_ref = 低分位/最小值，取 [z_ref, z_ref + margin]
        - 'max'  : 参考 z_ref = 高分位/最大值，取 [z_ref - margin, z_ref]
      根据你的坐标定义选择（若越小越近，则用 'min'）。
    depth_margin_mm : 薄层厚度（mm）。
    roi_rect_px : (x,y,w,h) 若给定，仅在该 ROI 内估计分位并输出掩码。
    open_k, close_k : 形态学开/闭核尺寸（像素）。为 None 则使用默认。
    use_percentile : True 时用分位数（更稳健），False 时用 min/max。
    qlo, qhi : 分位数阈值（百分数）。
    min_component_area_px : 过滤小噪声连通域的面积阈值。

    返回
    ----
    mask : (H,W) uint8，0/255 的“最近表面”薄层掩码。
    """
    H, W = height.shape[:2]
    ...
    # 2) 估计参考 z_ref
    if use_percentile:
        if str(z_select).lower().startswith('max'):
    ...
    # 4) 形态学净化（若工程里已有 morph_cleanup，则优先用之）
    def _morph_cleanup_local(mask_u8: np.ndarray, ok: int, ck: int) -> np.ndarray:
        res = mask_u8.copy()
        if ck and ck > 1:
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (int(ck), int(ck)))
            res = cv2.morphologyEx(res, cv2.MORPH_CLOSE, k, iterations=1)
        if ok and ok > 1:
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (int(ok), int(ok)))
            res = cv2.morphologyEx(res, cv2.MORPH_OPEN, k, iterations=1)
        return res

    try:
        # 若你的工程文件里已有 morph_cleanup(open_k, close_k) 签名相同，会优先调用
        m = morph_cleanup(m, open_k or 3, close_k or 5)  # type: ignore[name-defined]
    except Exception:
        m = _morph_cleanup_local(m, open_k or 3, close_k or 5)
...
def _valid_mask_xyz(P: np.ndarray) -> np.ndarray:
    ...
    if rpz is True:
        ...
        pos = np.count_nonzero(Z > 0);
        neg = np.count_nonzero(Z < 0)
        valid &= (Z > 0) if pos >= neg else (Z < 0)
    return valid


def compute_global_bounds(P_mach: np.ndarray, qlo: float, qhi: float, margin_mm: float) -> Tuple[
...
class DeviationController:
    ...
    def update(self, e_n: np.ndarray, normals: np.ndarray):
        ...
        self._prev_out = out
        stats = dict(mean=float(np.mean(e_n)), median=float(np.median(e_n)), p95=float(np.percentile(e_n, 95)),
                     n=int(len(e_n)))
        return out, stats
...
def draw_deviation_overlay(vis_top: np.ndarray, ref_xy: np.ndarray, actual_xy: np.ndarray,
    ...
    if e_n.size > 0 and ref_xy.size > 1:
        ...
        for k in range(0, len(idx), max(1, stride)):
            ...
            cv2.arrowedLine(out, tuple(b), tuple(t), (0, 255, 0) if abs(e_n[k]) < 0.5 else (0, 165, 255), 2,
...
def _bridge_endpoints(mask: np.ndarray, skel: np.ndarray, max_gap_px: int, max_bridges: int = 20) -> Tuple[
...
def fit_centerline_plus(mask: np.ndarray,
    ...
    skp[:, :, 2] = 0;
    skp[:, :, 1] = np.maximum(skp[:, :, 1], skp[:, :, 0])  # 偏黄
...
def main():
    ...
    try:
        while True:
            ...
            # 全场边界 + 像素尺寸自适应
            x0, x1, y0, y1 = compute_global_bounds(P_mach, CONFIG['bounds_qlo'], CONFIG['bounds_qhi'],
            ...
            if roi_mode == 'machine':
                cx, cy = CONFIG['roi_center_xy'];
                size = CONFIG['roi_size_mm']
                half = size * 0.5
            ...
            # --- v3: 最近表面薄层掩码（基于高度图） ---
            nearest_mask = extract_nearest_surface_mask_v3(
                height, mask_for_fit if roi_mode == 'machine' else mask_clean,
                z_select=CONFIG.get('z_select', 'max'),
                depth_margin_mm=CONFIG.get('depth_margin_mm', 3.0),
                roi_rect_px=roi_rect_px,
                open_k=CONFIG.get('morph_open', 3), close_k=CONFIG.get('morph_close', 5),
                use_percentile=CONFIG.get('nearest_use_percentile', True),
                qlo=CONFIG.get('nearest_qlo', 1.0), qhi=CONFIG.get('nearest_qhi', 99.0),
                min_component_area_px=CONFIG.get('min_component_area_px', 600)
            )
            cv2.imshow('NearestSurfaceMask', nearest_mask)

            # 中轴线拟合使用最近表面掩码
            mask_used = nearest_mask

            # 中轴线拟合（若开启 ROI 则用裁剪后掩码；否则直接用全场掩码，可能较慢/分叉多）
            # mask_used <- 已在上方改为最近表面掩码
            ...
            stats = dict(mean=0.0, median=0.0, p95=0.0, n=0)
            ...
            if key == ord('q'):
                ...
                CONFIG['roi_center_xy'] = [0.0, 0.0];
                CONFIG['roi_size_mm'] = 120.0
            elif roi_mode == 'machine':
                if key == ord('i'):
                    CONFIG['roi_center_xy'][1] += CONFIG['roi_size_mm'] * 0.1
                elif key == ord('k'):
                    CONFIG['roi_center_xy'][1] -= CONFIG['roi_size_mm'] * 0.1
                elif key == ord('j'):
                    CONFIG['roi_center_xy'][0] -= CONFIG['roi_size_mm'] * 0.1
                elif key == ord('l'):
                    CONFIG['roi_center_xy'][0] += CONFIG['roi_size_mm'] * 0.1
                elif key == ord('['):
                    CONFIG['roi_size_mm'] = max(20.0, CONFIG['roi_size_mm'] * 0.8)
                elif key == ord(']'):
                    CONFIG['roi_size_mm'] = min(1200.0, CONFIG['roi_size_mm'] / 0.8)
