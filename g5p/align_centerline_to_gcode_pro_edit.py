...
PARAMS = dict(
    ...
    # 纠偏应用策略：'invert'（默认，朝向理论轨迹纠偏）或 'follow'（直接跟随测得中心线）
    offset_apply_mode='invert',

    centerline_gcode='out/centerline.gcode',          # 可选：直接导出中心线轨迹
    export_centerline=False,                          # True 则同时导出中心线
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if guard_enable and (not guard_ok):
                    ...
                    # 计算沿轨迹的有符号法向偏移
                    delta_n_meas, dxy_vec = compute_offsets_along_gcode(centerline_xy, g_xy, N_ref)
                    if delta_n_meas.size > 0:
                        # === 1) 自动符号判定（可选，对“测得偏移”本身进行判断） ===
                        if PARAMS.get('auto_flip_offset', True):
                            # 使用中心线与原轨迹的 Y 中位差与法向偏移中位数符号比对；若不一致，翻转测得偏移的符号
                        ...
                        # === 3) 平滑 & 限幅（对应用偏移） ===
                        delta_n_apply = np.clip(moving_average_1d(delta_n_apply, PARAMS['guide_smooth_win']),
                                              -PARAMS['guide_max_offset_mm'], PARAMS['guide_max_offset_mm'])
                        g_xy_corr = g_xy[:len(delta_n_apply)] + N_ref[:len(delta_n_apply)] * delta_n_apply[:,None]
                        seg = np.linalg.norm(np.diff(g_xy[:len(delta_n_apply)], axis=0), axis=1)
                        s = np.concatenate([[0.0], np.cumsum(seg)])
                        save_offset_csv(s, delta_n_apply, N_ref[:len(delta_n_apply)]*delta_n_apply[:,None], PARAMS['offset_csv'])
