    ...
    while True:
        ...
        if len(image_list) == 2:
            ...
            if registration_mode == 0:  # 深度图映射到彩色图坐标系
                ...
                # --- 新增步骤 1: 提取最近表面的掩码 ---
                # 你需要为 depth_margin 设置一个合适的值，比如 5-10mm，取决于层高和噪声
                surface_mask, z_min = extract_nearest_surface_mask(roi_cloud, depth_margin=5)
                ...
                # --- 新增步骤 2: 使用掩码过滤点云 ---
                # 创建一个新的点云，只包含最近表面的点
                nearest_roi_cloud = roi_cloud.copy()
                # 将不属于最近表面的点设置为无效值(0,0,0)，这样它们在拟合时会被忽略
                nearest_roi_cloud[surface_mask == 0] = [0, 0, 0]

                # process
                # --- 修改步骤 3: 对过滤后的点云进行拟合 ---
                plane_model, inlier_mask, height_map, conter_vis = fit_plane_and_extract_height_map(
                    nearest_roi_cloud,  # 使用只包含最近表面的点云
                    distance_threshold=1.8,  # 正方形物体所用
                    visualize=True
                )
